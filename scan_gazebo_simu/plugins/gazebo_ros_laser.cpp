...
namespace gazebo {
  ...
  void GazeboRosLaser::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf) {
    ...
    if (!_sdf->HasElement("frameName")) {
      ROS_INFO_NAMED("_laser", " laser plugin missing <frameName>, defaults to /world");
    ...
    if (!_sdf->HasElement("topicName")) {
      ROS_INFO_NAMED("_laser", " laser plugin missing <topicName>, defaults to /world");
    ...
    if (!_sdf->HasElement("gaussianNoise")) {
      ROS_INFO_NAMED("_laser", " laser plugin missing <gaussianNoise>, defaults to 0.0");
    ...
    if (!_sdf->HasElement("hokuyoMinIntensity")) {
      ROS_INFO_NAMED("_laser", " laser plugin missing <hokuyoMinIntensity>, defaults to 101");
    ...
    ROS_DEBUG_NAMED("_laser", "gazebo_ros_laser plugin should set minimum intensity to %f due to cutoff in hokuyo filters." , this->hokuyo_min_intensity_);

    if (!_sdf->HasElement("updateRate")) {
      ROS_INFO_NAMED("_laser", " laser plugin missing <updateRate>, defaults to 0");
      this->update_rate_ = 0;
    }
    else {
    ...
    if (!ros::isInitialized()) {
      ROS_FATAL_STREAM_NAMED("_laser", "A ROS node for Gazebo has not been initialized, unable to load plugin. "
    ...
    this->callback_laser_queue_thread_ = boost::thread( boost::bind( &GazeboRosLaser::LaserQueueThread,this ) );
  ...
  void GazeboRosLaser::OnNewLaserScans() {
    if (this->topic_name_ != "") {
      ...
      ROS_INFO_NAMED("_laser", "gazebo_ros_laser topic name not set");
    }
  }
  ...
  void GazeboRosLaser::OnStats( const boost::shared_ptr<msgs::WorldStatistics const> &_msg) {
